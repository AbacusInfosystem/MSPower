...

{
    public class ProductDetailRepo
    {
        ...
        {
            ProductCategoryColumnMappingInfo product_Category_Column_Mapping = new ProductCategoryColumnMappingInfo();
            ...
            param.Add(new SqlParameter("@product_Category_Column_Id", product_Category_Column_Id));

            DataTable dt = _sqlDataAccess.ExecuteDataTable(param, StoredProcedures.Get_Product_Category_Column_By_Id_sp.ToString(), CommandType.StoredProcedure, _con);
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    product_Category_Column_Mapping.Product_Category_Column_Mapping_Id = Convert.ToInt32(dr["Product_Category_Column_Mapping_Id"]);

                    product_Category_Column_Mapping.Product_Category_Id = Convert.ToInt32(dr["Product_Category_Id"]);

                    product_Category_Column_Mapping.Volts = Convert.ToInt32(dr["Volts"]);

                    product_Category_Column_Mapping.Product_Column_Ref_Id = Convert.ToInt32(dr["Product_Column_Ref_Id"]);
        ...
        public ProductCategoryInfo Get_Product_Category_By_Id(int product_Category_Id)
        {
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    product_Category.Product_Category1 = Convert.ToString(dr["Product_Category"]);
